from bameditor.common.bamobject import Mutation, Haplotype
from bameditor.common.methods import calPolymorphism
import pysam
...
def check_polymorphism(bam, ref, mutinfo, maxsnpfrac):
    ...
    for pos in range(mutinfo[1] - 1, mutinfo[2]):
        ...
        for nul in 'ATCG':
            if poly_statistic[pos][nul] == ref_nul:
                continue
            if reads_num != 0:
    ...
    with open(mut_file, 'r') as fin_mut:
        for lineId, mutline in enumerate(fin_mut):
            ...
            if mutinfo[type_id] in ["snv", "del", "sub"] and not check_polymorphism(bam, ref, mutinfo, snpfrac):
                invalid_log.info(
                    "line %s invalid in mutFile: snp fraction bigger than run_args snvfrac(%s) " % (lineId, snpfrac))
                continue
    ...
    for mut in mut_list:
        if not cur_end:
            ...
            if mut.start <= cur_end:
                invalid_log.info(
                    "mutation invalid(overlapped by other mutations): %s:%s-%s" % (mut.chrom, mut.start, mut.end))
                continue
            hap_mut_list.append(mut)
        else:
