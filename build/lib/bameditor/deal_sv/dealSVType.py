    ...
    for sv_info in sv_list:
        ...
        if not sv_info.sv_type == "cnv":
            ...
            if start_coverage < mindepth or end_coverage < mindepth:
                invalid_log.info(str(sv_info), "coverage too small", start_coverage, end_coverage)
                continue
            if start_coverage_deal < minmutreads or end_coverage_deal < minmutreads:
                invalid_log.info(str(sv_info), "coverage deal too small", start_coverage_deal, end_coverage_deal)
                continue
...
def deal_translocation_unbalance(bam, ref, svinfo, is_single, readLength, tempDir, insertSize, minmapq,
    ...
    if is_single:
        ...
        modify_reads_type2 = deal_type2(ref, [], trans_reads_type2_left, freq, trans_start, trans_end, svtype,
                                        subPos=info)
        modify_reads_type3 = deal_type3(trans_reads_type3_left, trans_reads_type3_left, freq, insertSize, trans_start,
                                        trans_end, svtype, supple1=reads_type3_left, supple2=reads_type3_right,
                                        subPos=info)
        if modify_reads_type3 is False:
...
def deal_duplication(bam, ref, svinfo, is_single, readLength, tempDir, insertSize, minmapq, is_multmapfilter):
    ...
    if is_single:
        ...
        add_reads_type3 = deal_type3(reads_type3_left, reads_type3_right, freq_dup, insertSize, start, end, svtype)
        if add_reads_type3 is False:
            return False, "The reads are not satisfied to modify, see more details in log file"
...
def deal_deletion(bam, ref, svinfo, is_single, readLength, tempDir, insertSize, minmapq, is_multmapfilter):
    ...
    if is_single:
        ...
        if freq > 1:
            freq = 1
        modify_reads_type6 = deal_type6(ref, reads_type6_left, reads_type6_left, freq, start, end, svtype, seqer=None)
        delete_reads_type7 = deal_type7(reads_type7, freq, svtype)
        ...
        modify_reads_type3 = deal_type3(reads_type3_left, reads_type3_right, freq, insertSize, start, end, svtype,
                                        supple1=supple1)
        if modify_reads_type3 is False:
            return False, "The reads are not satisfied to modify, see more details in log file"
        ...
        # modify, delete, add
        return modify_reads_type1 + modify_reads_type3, delete_reads_type2 + delete_reads_type4 + delete_reads_type5, []
...
def deal_translocation_balance(bam, ref, svinfo, is_single, readLength, tempDir, insertSize, minmapq, is_multmapfilter):
    ...
    if is_single:
        ...
        modify_reads_type3 = deal_type3(reads_type3_left, reads_type3_right, freq, insertSize, start, end, svtype,
                                        subPos=trans_info, supple1=trans_reads_type3_left,
                                        supple2=trans_reads_type3_right)
        trans_modify_reads_type3 = deal_type3(trans_reads_type3_left, trans_reads_type3_right, freq, insertSize,
                                              trans_start, trans_end, svtype, subPos=info, supple1=reads_type3_left,
                                              supple2=reads_type3_right)
        if modify_reads_type3 is False or trans_modify_reads_type3 is False:
            return False, "The reads are not satisfied to modify, see more details in log file"
