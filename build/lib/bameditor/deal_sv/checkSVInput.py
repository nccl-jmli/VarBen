from bameditor.common.bamobject import SV
import pysam
import traceback
...
def check_sv(sv_info, chrom_len):
    ...
    try:
        ...
        if sv_type == "inv":
            ...
            if end - start < min_size or not 1 <= dup_num <= 5:
                raise Exception("duplication is not in correct format")
        elif sv_type == "cnv":
            if (sv_info[cnv_type_id] == "loss" and float(sv_info[freq_id]) <= 1) or (sv_info[cnv_type_id] == "gain" and float(sv_info[freq_id]) >= 1):
                pass
            else:
                raise Exception("cnv is not in correct format")
        elif sv_type == "trans_balance":
            trans_chr, trans_start, trans_end = sv_info[trans_chr_id], int(sv_info[trans_start_id]), int(sv_info[trans_end_id])
